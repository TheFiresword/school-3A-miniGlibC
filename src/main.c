...
void test4(){
   ...
   if(my_file!=NULL){
        char* buffer=mini_calloc(sizeof(char),100);
        int tempo=mini_fread(buffer,sizeof(char),100,my_file);
        
        while(tempo!=0 && tempo!=-1){
            mini_printf("On a lu: ");
            mini_printf(buffer);
            mini_printf("\n");
            tempo=mini_fread(buffer,sizeof(char),100,my_file);
        }
        mini_free(buffer);
   }
   mini_fclose(my_file);
